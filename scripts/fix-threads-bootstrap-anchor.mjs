...
source = source.replace(
  ...
  "  platform: 'instagram' | 'facebook' | 'youtube' | 'tiktok' | 'linkedin' | 'threads';\n  display_name: string;",
);
...
const after = `    db.prepare(
      \`SELECT id, platform, display_name, handle, status, last_synced_at
       ...
       WHERE workspace_id = ?
       ORDER BY platform, display_name\`,
    ).bind(principal.workspaceId, principal.workspaceId, principal.workspaceId, principal.workspaceId),`;

if (!source.includes(before)) {
  throw new Error('Threads bootstrap fix failed: Facebook/LinkedIn-aware bootstrap query not found.');
}

source = source.replace(before, after);
source += '\n// SC_THREADS_BOOTSTRAP_V1\n';
fs.writeFileSync(path, source);
console.log('Threads bootstrap aggregation applied after Facebook and LinkedIn storage patches.');
